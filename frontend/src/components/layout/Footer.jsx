...
export default function Footer() {
  return (
    <footer className="bg-ink text-white pt-16 pb-8 px-10">
      <div className="grid grid-cols-4 gap-10 mb-12">
        <div>
          <div className="font-serif text-2xl font-light mb-3">TWEBAZE<span className="text-gold">arts</span></div>
        ...
        {[
          ...
          <div key={title}>
            ...
            <div className="flex flex-col gap-2.5">
              {links.map(([to, label]) => (
                <Link key={label} to={to} className="text-sm text-mist hover:text-white transition-colors">{label}</Link>
              ))}
            </div>
          </div>
        ))}
      </div>
      <div className="border-t border-white/10 pt-6 flex justify-between text-[11px] text-stone">
        <span>© {new Date().getFullYear()} TWEBAZEarts. All rights reserved.</span>
        <span>Built for African art, by Africans.</span>
      </div>
    </footer>
  );
}