...
export default function Gallery() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="min-h-[calc(100vh-64px)] grid grid-cols-2 pt-16">
        <div className="flex flex-col justify-center px-16 py-20">
          <p className="text-[11px] tracking-[.2em] uppercase text-gold mb-6">Kampala · East Africa</p>
          <h1 className="font-serif text-[clamp(52px,6vw,88px)] leading-[1.05] font-light mb-8">
            Art that<br /><em className="text-stone italic">speaks</em><br />across walls
          </h1>
          <p className="text-[15px] text-stone max-w-sm leading-[1.8] mb-12">
            A curated platform celebrating Uganda's most compelling visual artists.
          </p>
          <div className="flex gap-4">
            <a href="#gallery" className="bg-ink text-white px-8 py-3.5 text-[11px] tracking-widest uppercase hover:bg-gold transition-colors">
              Explore Collection
            </a>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden bg-smoke flex items-center justify-center">
          {artworks[0] ? (
      ...
      {/* Stats */}
      <div className="bg-ink flex justify-center gap-20 py-8">
        {[['12', 'Curated Artists'], ['48', 'Active Works'], ['6', 'Disciplines'], ['100%', 'East African']].map(([n, l]) => (
          <div key={l} className="text-center">
            <div className="font-serif text-3xl font-light text-white">{n}</div>
            <div className="text-[10px] tracking-[.15em] uppercase text-mist mt-1">{l}</div>
          </div>
        ))}
      </div>
      ...
      <section id="gallery" className="px-10 py-20">
        ...
        <div className="flex gap-2 flex-wrap mb-10">
          {FILTERS.map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-5 py-2 text-[11px] tracking-widest uppercase border transition-colors ${
        ...
        {isLoading ? (
          <div className="gallery-grid bg-ash">
